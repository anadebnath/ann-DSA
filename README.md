...
For around **1 billion sorted elements**, binary search needs only about **30 comparisons** in the worst case because:

```text
2³⁰ ≈ 1 billion
```

That's the power of logarithmic complexity. 🚀

---

### 🔹 2. Recursion uses the call stack
...
Without a proper base condition, recursion can continue until the program exhausts available stack space.

---

### 🔹 3. Quick Sort can be very fast

Although its worst-case complexity is **O(n²)**, Quick Sort commonly performs very well in practice when the pivot selection and partitioning are favorable.

---

### 🔹 4. Merge Sort has a predictable complexity

Merge Sort maintains **O(n log n)** time complexity in its standard form, making it useful when predictable performance is important.

---

### 🔹 5. Big-O describes growth

Big-O notation isn't simply about how fast a program runs on one machine.

It describes how the amount of work **grows as the input size increases**.
